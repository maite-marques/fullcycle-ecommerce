...
func (pd *ProductDB) GetProductByCategoryID(categoryID string) ([]*entity.Product, error) {
	rows, err := pd.db.Query("SELECT id, name, description, price, category_id, image_url, FROM products WHERE catedory_id = ?", categoryID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (pd *ProductDB) CreateProduct(product *entity.Product) (*entity.Product, error) {
	...
	return product, nil // retornando o mesmo produto que recebeu por parâmetro // poderia retornar só o id tmb.
}